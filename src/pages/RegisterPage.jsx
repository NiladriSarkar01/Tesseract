...
// --- Landing/Events Page Component ---

const EventsPage = ({ onNavigateToRegister }) => {
  ...
  return (
    <div className="relative z-10 container mx-auto px-6 pt-10 pb-20 animate-in fade-in duration-700 h-screen flex flex-col">
      <style>{`
      ...
      <div className="text-center mb-8 shrink-0">
        <div className="inline-flex items-center gap-3 px-6 py-2 rounded-full bg-black/50 border border-red-500/20 backdrop-blur-md mb-4 text-pink-200 text-xs font-bold tracking-[0.2em] uppercase shadow-[0_0_10px_rgba(239,68,68,0.2)]">
        ...
        <h1 className="text-4xl md:text-5xl font-black mb-4 tracking-tighter text-white">
          EXPLORE THE <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-orange-600">UNIVERSE</span>
        </h1>
      </div>

      {/* Filter Bar */}
      <div className="mb-8 shrink-0">
        <div className="bg-black/70 backdrop-blur-xl border border-white/10 p-2 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4 max-w-4xl mx-auto shadow-2xl shadow-black">
          <div className="flex gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-hide">
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() => setActiveTab(cat)}
                className={`px-6 py-2 rounded-xl text-sm font-medium transition-all whitespace-nowrap ${activeTab === cat
          ...
          <div className="relative w-full md:w-64">
            ...
            <input
              ...
              className="w-full bg-[#050000] border border-white/10 rounded-xl py-2 pl-10 pr-4 text-sm text-white focus:outline-none focus:border-red-500/50 transition-colors"
            />
          </div>
        </div>
      </div>

      {/* Cinematic List Container */}
      {filteredEvents.length > 0 ? (
        <div className="flex-grow relative w-full max-w-4xl mx-auto overflow-hidden">
          {/* Top Fade Gradient */}
          <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-b from-[#050000] via-[#050000]/80 to-transparent z-20 pointer-events-none"></div>

          {/* Bottom Fade Gradient */}
          <div className="absolute bottom-0 left-0 w-full h-32 bg-gradient-to-t from-[#050000] via-[#050000]/80 to-transparent z-20 pointer-events-none"></div>

          {/* Scrollable List */}
          <div className="h-full overflow-y-auto pb-40 pt-40 px-4 scrollbar-hide text-center space-y-12">
            {filteredEvents.map((event) => (
              <div
                key={event.id}
                onClick={() => onNavigateToRegister(event.id)}
                className="group cursor-pointer transition-all duration-500 hover:scale-105 opacity-40 hover:opacity-100 flex flex-col items-center"
              >
                <h2 className="text-3xl md:text-5xl font-black text-transparent bg-clip-text bg-white group-hover:bg-gradient-to-r group-hover:from-red-500 group-hover:to-orange-500 transition-all tracking-tight uppercase">
                  {event.title}
                </h2>
                <div className="flex items-center gap-4 mt-2 text-gray-500 group-hover:text-red-300 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0">
                  <span className="flex items-center gap-1 text-sm tracking-widest uppercase"><Zap size={12} /> {event.category}</span>
                  <span className="w-1 h-1 rounded-full bg-red-500"></span>
                  <span className="flex items-center gap-1 text-sm tracking-widest uppercase"><Calendar size={12} /> {event.date}</span>
                  {/* Show Mode Badge */}
                  <span className="w-1 h-1 rounded-full bg-red-500"></span>
                  <span className="flex items-center gap-1 text-sm tracking-widest uppercase">
                    {event.participationMode === 'both' ? 'Solo / Team' : event.participationMode}
                  </span>
                </div>
              </div>
            ))}
        ...
        <div className="flex-grow flex items-center justify-center">
          <div className="text-center py-20 bg-white/5 rounded-3xl border border-white/5 max-w-md mx-auto">
            <Search size={48} className="mx-auto text-red-600 mb-4" />
            <h3 className="text-xl font-bold text-white mb-2">No events found</h3>
            <p className="text-gray-400">Try adjusting your filters or search terms.</p>
            <button
              onClick={() => { setActiveTab('All'); setSearchQuery('') }}
              className="mt-6 px-6 py-2 bg-red-600 text-white rounded-xl font-bold hover:bg-red-500 transition-colors"
            >
              Clear Filters
            </button>
          </div>
...
const App = () => {
  const [currentPage, setCurrentPage] = useState('events'); // 'events' | 'register'
  const [selectedEventId, setSelectedEventId] = useState(null);

  const navigateToRegister = (eventId) => {
  ...
  return (
    <div className="min-h-screen bg-[#050000] text-white font-sans selection:bg-red-600 selection:text-white overflow-hidden">
      ...
      <div className="fixed bottom-0 right-0 w-[500px] h-[500px] bg-red-900/20 rounded-full blur-[120px] pointer-events-none z-0"></div>

      {currentPage === 'events' ? (
        <EventsPage onNavigateToRegister={navigateToRegister} />
      ) : (
        <RegisterPage
