...
// --- DATA GENERATOR ---
const generateHiveData = (count = 25) => {
  // Increased count to fill space
  const baseImages = [
...
const HexCard = ({ image }) => {
  return (
    ...
    <div className="group relative w-16 h-20 md:w-24 md:h-28 flex items-center justify-center z-10 hover:z-50 transition-all duration-300 ease-[cubic-bezier(0.25,1,0.2,1)] hover:scale-[2.8]">
      {/* 1. Glow Effect (Hover) */}
      <div className="absolute inset-0 bg-cyan-500/60 blur-xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none scale-110" />

      {/* 2. The Border Layer (Background Color) */}
      <div
        className="absolute inset-0 bg-cyan-900/30 group-hover:bg-white transition-colors duration-300"
    ...
    <section className="min-h-screen bg-transparent text-white py-24 font-sans relative overflow-hidden flex flex-col items-center justify-center">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="mb-12 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-cyan-900/10 border border-cyan-500/20 mb-6">
            <Zap
              size={14}
              className="text-blue-500 fill-current animate-pulse"
            />
