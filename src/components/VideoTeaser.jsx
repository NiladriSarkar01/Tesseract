...
import { Play, Volume2, VolumeX, Zap, ChevronRight, RefreshCw, Terminal, Shield, Cpu, Gamepad2, Music, Code, Globe, Lock, Satellite, Wifi, AlertTriangle, Radio, Crosshair, Binary } from 'lucide-react';
...
const VideoTeaser = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setDecodeString(Array(12).fill(0).map(() => chars[Math.floor(Math.random() * chars.length)]).join(''));
    }, 60);
  ...
  const renderIdle = () => (
    <div className="absolute inset-0 flex items-center justify-center bg-black z-50 cursor-pointer group" onClick={startTeaser}>
      ...
      <div className="absolute bottom-20 text-center">
        <p className="text-cyan-500 font-mono text-sm tracking-[0.3em] animate-pulse">INITIALIZE_TRAILER_SEQUENCE</p>
      </div>
    </div>
  );
    ...
    <div className="absolute inset-0 bg-black flex flex-col items-center justify-center font-mono z-40">
      <div className="w-80 mb-6">
        ...
        <div className="h-2 w-full bg-cyan-900/20 rounded-sm overflow-hidden border border-cyan-900/50">
          <div className="h-full bg-cyan-500 transition-all duration-75 ease-linear" style={{ width: `${progress}%` }}></div>
        </div>
      </div>
      <div className="text-[10px] text-gray-500 space-y-1 font-mono h-32 overflow-hidden w-80">
        <p>{'>'} BIOS_CHECK... OK</p>
        <p>{'>'} MEMORY_ALLOCATION... {progress * 12}MB</p>
        <p className={progress > 20 ? 'text-emerald-400' : 'hidden'}>{'>'} DECRYPTING_CORE_ASSETS...</p>
        <p className={progress > 40 ? 'text-emerald-400' : 'hidden'}>{'>'} LOADING_MODULES [GRAPHICS, AUDIO, NETWORK]...</p>
        <p className={progress > 60 ? 'text-emerald-400' : 'hidden'}>{'>'} ESTABLISHING_SECURE_UPLINK...</p>
        <p className={progress > 80 ? 'text-emerald-400' : 'hidden'}>{'>'} OVERRIDING_SAFETY_PROTOCOLS...</p>
      </div>
    </div>
  );
  ...
  const renderUplink = () => (
    <div className="absolute inset-0 bg-[#020408] flex flex-col items-center justify-center z-40">
      <div className="relative w-64 h-64 border border-cyan-500/20 rounded-full flex items-center justify-center mb-8 animate-[spin_10s_linear_infinite]">
        ...
        <Satellite size={32} className="absolute top-0 right-0 text-white animate-bounce" />
      </div>
      <h2 className="text-3xl font-black text-white tracking-widest mb-2">ESTABLISHING UPLINK</h2>
  ...
  const renderSecurity = () => (
    <div className="absolute inset-0 bg-[#000505] flex flex-col items-center justify-center z-40 font-mono">
      <div className="relative mb-8">
        ...
        <Shield size={80} className="text-emerald-500 relative z-10 animate-bounce" />
      ...
      <h2 className="text-2xl text-emerald-500 font-bold tracking-widest mb-2 animate-pulse">{securityText}</h2>
      <div className="flex gap-1 h-1 w-24 mx-auto">
         {[...Array(5)].map((_,i) => (
           <div key={i} className="flex-1 bg-emerald-500/50 animate-[pulse_0.5s_ease-in-out_infinite]" style={{ animationDelay: `${i * 0.1}s`}}></div>
         ))}
      </div>
    </div>
  );
    ...
    <div className="absolute inset-0 bg-red-950 flex flex-col items-center justify-center z-40">
       ...
       <div className="mt-8 flex items-center gap-4">
         ...
         <div className="text-left">
            ...
            <p className="text-white font-bold animate-pulse">BYPASSING FIREWALL...</p>
         </div>
       </div>
    </div>
  );
    ...
    <div className="absolute inset-0 bg-black flex items-center justify-center z-40">
       ...
       <div className="relative w-full h-full flex items-center justify-center">
          ...
          <div className="relative z-0 w-full h-full bg-red-900/10 animate-pulse flex items-center justify-center">
             <Crosshair size={200} className="text-red-500 opacity-80 animate-[spin_0.5s_ease-out]" />
             <h2 className="absolute mt-32 text-4xl font-black text-red-500 tracking-[0.5em] animate-bounce">TARGET_LOCKED</h2>
          </div>
          {/* HUD Corners */}
  ...
  const renderHype = () => (
    <div className="absolute inset-0 bg-black flex items-center justify-center z-40 overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 right-0 h-32 flex items-end justify-center gap-1 opacity-50">
         {[...Array(20)].map((_, i) => (
            <div key={i} className="w-2 bg-cyan-500 animate-[bounce_0.5s_infinite]" style={{ height: `${Math.random() * 100}%`, animationDelay: `${Math.random() * 0.5}s` }}></div>
         ))}
      </div>
    </div>
  );
  ...
  const renderFeatures = () => {
    ...
    return (
      <div className="absolute inset-0 bg-[#0a0a0a] flex flex-col items-center justify-center z-40">
         ...
         <h2 key={activeFeature} className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 to-white tracking-tighter uppercase animate-in slide-in-from-bottom duration-200">
    ...
    <div className="absolute inset-0 bg-black flex flex-col items-center justify-center z-40">
       <div className="relative border-y-4 border-blue-600 py-12 w-full text-center bg-blue-900/20 backdrop-blur-lg animate-in zoom-in duration-700">
          ...
          <div className="overflow-hidden">
             <p className="text-xl md:text-2xl text-cyan-400 font-mono uppercase tracking-widest animate-in slide-in-from-top duration-1000 delay-300">SODEPUR, PANIHATI, KOLKATA</p>
          </div>
       </div>
    </div>
  );
    ...
    <div className="absolute inset-0 bg-[#050000] flex flex-col items-center justify-center z-40 animate-in fade-in duration-1000">
       ...
       <div className="text-center relative z-10">
          ...
          <h2 className="text-5xl md:text-7xl font-black text-white mb-12 tracking-tight">
            ARE YOU <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 to-emerald-500 animate-pulse">READY?</span>
          </h2>
          ...
          <div className="flex flex-col md:flex-row gap-6 justify-center items-center">
            ...
            <button 
              onClick={() => { setIsPlaying(false); setTimeout(() => setPhase('idle'), 100); setLogs([]); }}
    ...
    <div className="relative w-full h-[93vh] bg-black overflow-hidden font-sans selection:bg-cyan-500 selection:text-white">
      ...
      {phase !== 'idle' && (
        <div className="absolute bottom-8 left-8 z-[60] font-mono text-[10px] text-cyan-500/70 pointer-events-none flex flex-col-reverse">
          {logs.map((log) => (
            <div key={log.id} className={`animate-in slide-in-from-left fade-in duration-300 ${log.type === 'error' ? 'text-red-500' : log.type === 'success' ? 'text-green-500' : ''}`}>
              {`> ${log.text}`}
            </div>
          ))}
        </div>
